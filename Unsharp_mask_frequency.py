...
def UnsharpTrans(img,d=500):
    ...
    return np.clip(img+new_flp,0,255).astype(np.uint8)
...
# som = Slider(om, r'D0', 450.0, 2000.0, valinit=1000.0)
# def update(val):
#     s = som.val
#     cnt=0
#     for i in range(1,len(ax_lt),2):
#         ax_lt[i].imshow(UnsharpTrans(img_lt[cnt],d=s),cmap='gray')
#         cnt+=1
